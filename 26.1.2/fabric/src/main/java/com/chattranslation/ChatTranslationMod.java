                ...
                .thenAccept(translatedText -> {
                    ...
                    if (translatedText == null || translatedText.startsWith("[")) {
                        ...
                        client.execute(() -> {
                            ...
                            if (client.gui != null) {
                                client.gui.getChat().addMessage(fallbackMessage, (MessageSignature) null, GuiMessageSource.SYSTEM, GuiMessageTag.system());
                            }
                        });
                        return;
                    }
                    if (!translatedText.equals(originalText)) {
                        Minecraft client = Minecraft.getInstance();
                        client.execute(() -> {
                            Component translationMessage = formatTranslationMessage(originalText, translatedText);
                            LOGGER.info("[ChatTranslation][debug:hud-add] message='{}'", translationMessage.getString());
                            if (client.gui != null) {
                                client.gui.getChat().addMessage(translationMessage, (MessageSignature) null, GuiMessageSource.SYSTEM, GuiMessageTag.system());
                            } else {
